...
export default function TimeTracker() {
  ...
  const displayTime = isOnBreak
    ? (now - (breaks[breaks.length - 1]?.start || now)) / 1000
    : actualWorkedTime;
        ...
        <Card className="w-full mx-auto shadow-lg">
          <CardHeader>
            <CardTitle className="flex justify-between items-center">
              <span>{todayDay}</span>
              <span className="bg-red-500 p-2 rounded-sm text-white">
                Clockin Time:{" "}
          ...
          <CardContent className="text-center py-6 relative">
            ...
            {isOnBreak && (
              <p className="mt-2 text-sm text-red-600 font-medium uppercase tracking-wide">
                On Break
              </p>
            )}
          </CardContent>
            ...
            <CardContent>
              <div className="flex flex-col gap-2">
                ...
                <Textarea
                  id="dwr"
                  value={dwr}
                  onChange={(e) => setDwr(e.target.value)}
              ...
              <ul className="list-disc space-y-1">
                {breaks.map((b, i) => (
                  <li key={i} className="flex justify-between text-sm">
                    <span>
                      {dayjs(b.start).format("HH:mm:ss")} -{" "}
                      {b.end ? dayjs(b.end).format("HH:mm:ss") : "Ongoing"}
                    </span>
                    <span className="ml-2 font-mono">
                      {formatTime(((b.end ?? now) - b.start) / 1000)}
                    </span>
                  </li>
                ))}
              </ul>
